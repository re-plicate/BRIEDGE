...
from communication.python.cortex import Cortex
import numpy as np
import shutil
record = []
...
class Subcribe():
    ...
    """
    def __init__(self,task_name, prt_list, app_client_id, app_client_secret, **kwargs):
        """
    ...
    def on_new_pow_data(self, *args, **kwargs):
        ...
        """
        data = kwargs.get('data')
        print('Emotiv EPOC Collecting-----------', data['pow'])
        record.append(data['pow'])
        if len(record) == 80:
            with open(r'C:\Users\欧阳\PycharmProjects\multi-brain-system\communication\data_record\Emotiv/{0}_epoc.txt'.format(self.task_name), 'w') as file:
                for item in record:
                    file.write("%s\n" % str(item).strip("[]"))
...
def main(task_name):
    ...
    your_app_client_id = ' '  # your emotiv id
    your_app_client_secret = ' '  # your emotiv secret


    s = Subcribe(task_name, print_list, your_app_client_id, your_app_client_secret)
...
def copy(task_name):
    shutil.copy2('./{0}_epoc.txt'.format(task_name), "../../data_record/Emotiv")
    print('copy finished')
